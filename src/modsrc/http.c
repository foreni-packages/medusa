...
#define MODULE_VERSION_SVN "$Id: http.c 1259 2009-11-16 04:19:46Z jmk $"
...
void showUsage()
{
  ...
  writeVerbose(VB_NONE, "  DOMAIN:? [optional] (BASIC/NTLM)");
...
int initModule(_MODULE_DATA *_psSessionData, sLogin* _psLogin)
{
  ...
  while (nState != MSTATE_COMPLETE)
  {
    ...
    case MSTATE_NEW:
      ...
      {
        getAuthType(hSocket, _psSessionData, &_psLogin, params.nPort);
...
int sendAuthDigest(int hSocket, _MODULE_DATA* _psSessionData, char* szLogin, char* szPassword)
{
  ...
  char *szMethod = "GET";
  char *szQop = NULL;
  char *szURI = _psSessionData->szDir;
  char *szOpaque = NULL;
  HASHHEX HA1;
  HASHHEX HA2 = "";
  HASHHEX Response;
...
int tryLogin(int hSocket, _MODULE_DATA* _psSessionData, sLogin** login, int nPort, char* szLogin, char* szPassword)
{
  ...
  else 
  {
    ...
    writeError(ERR_DEBUG_MODULE, "Login failed: error code was %.3s", pTemp);
