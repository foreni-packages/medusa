...
#define MODULE_VERSION_SVN "$Id: mssql.c 1225 2009-10-13 18:55:27Z jmk $"
  ...
  { 
    ...
    else if (bufReceive[0] == 0x05)
    {
      ...
      while (szTmp = strstr(szTmp, "ServerName;"))
      {
        ...
        if ((szTmp2 = strstr(szTmp, ";tcp;")) == NULL)
        {
          writeError(ERR_ERROR, "[%s] SQL server instance %d on host %s - Failed to locate service port. Server may be configured to \"hide\" port. Setting default hidden value of 2433.", MODULE_NAME, nSQLInstance, _psLogin->psServer->pHostIP);
          nPort = 2433;
        }
        else
        {
          ...
          if (nSQLInstance == 1)
            ...
            writeError(ERR_ERROR, "[%s] Additional SQL server identified (NOT TESTED) - server %s on port %d/tcp", MODULE_NAME, _psLogin->psServer->pHostIP, nPort);
        }
