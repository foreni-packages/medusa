...
#define MODULE_VERSION_SVN "$Id: ssh.c 1295 2010-02-04 19:15:40Z jmk $"
...
#ifdef LIBSSH2_GCRYPT
  writeVerbose(VB_NONE, "");
  writeVerbose(VB_NONE, "The default build of Libssh2 is to use OpenSSL for crypto. Several Linux");
  writeVerbose(VB_NONE, "distributions (e.g. Debian, Ubuntu) build it to use Libgcrypt. Unfortunately,");
  writeVerbose(VB_NONE, "the implementation within Libssh2 of libgcrypt appears to be broken and is");
  writeVerbose(VB_NONE, "not thread safe. If you run multiple concurrent Medusa SSH connections, you");
  writeVerbose(VB_NONE, "are likely to experience segmentation faults. Please help Libssh2 fix this");
  writeVerbose(VB_NONE, "issue or encourage your distro to use the default Libssh2 build options.");
  writeVerbose(VB_NONE, "");
#endif
...
{
  /* Starting with libssh2 0.18 I'm getting a double free with this... */
  //free(ptr);
}
    ...
    switch (nState)
    {
      case MSTATE_NEW:
        ...
        */
        ...
        session = libssh2_session_init_ex(ssh2_init_alloc_cb, ssh2_init_free_cb, ssh2_init_realloc_cb, &ssh2_session_data);
        if (!session)
        {
