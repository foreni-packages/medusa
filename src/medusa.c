...
#define VERSION_SVN "$Id: medusa.c 1271 2009-11-25 00:24:58Z jmk $" 
...
int checkOptions(int argc, char **argv, sAudit *_psAudit)
{
  ...
  while ((opt = getopt(argc, argv, "h:H:u:U:p:P:C:O:e:M:m:g:r:R:t:T:n:bqdsLfFVv:w:Z:")) != EOF)
  {
    switch (opt)
    {
    case 'h':
...
/* gethostbyname_r does not appear to be present on NetBSD... we need to move away from gethostbyname/gethostbyname_r... */
#ifdef HAVE_GETHOSTBYNAME_R
  while ((res = gethostbyname_r(_psServer->psHost->pHost, &sHostent, tmphstbuf, hstbuflen, &hp, &herr)) && (errno == ERANGE))
  {
    writeError(ERR_DEBUG_SERVER, "Increasing buffer size for host name resolution.");
    hstbuflen *= 2;
  ...
  else
  {
    ...
    writeError(ERR_DEBUG_SERVER, "Set host IP: %s", _psServer->pHostIP);
...
#else
  psHostent = gethostbyname(_psServer->psHost->pHost);
  if (!psHostent) {
    writeError(ERR_CRITICAL, "Failed to resolve hostname: %s", _psServer->psHost->pHost);
    return;
  }
  ...
  {
    _psServer->pHostIP = inet_ntoa(* (struct in_addr *) psHostent->h_addr);
    writeError(ERR_DEBUG_SERVER, "Set host IP: %s", _psServer->pHostIP);
  ...
  */
  iLoginId = 0;
  while (_psServer->iCredentialsMissed > 0)
  {
